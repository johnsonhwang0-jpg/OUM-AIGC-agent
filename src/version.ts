...
export const VERSION_HISTORY: VersionEntry[] = [
  ...
  },
  {
    ...
    changes: [
      ...
      "Codex Debug 面板增强（events + artifacts）：新增 codexEvents/codexArtifacts/codexArtifactName 状态。轮询 status 时并行拉 /api/codex-build/:runId/events（不阻塞，失败容忍），展示 Agent 最近 15 条事件流（type + message/text）。完成后拉 /api/codex-build/:runId/artifacts 列出产物文件（名称+大小），标记实际下载的 HTML artifact。Raw Response 区显示 artifact 来源徽章（artifact output/index.html · 从产物下载 HTML）。",
      "后端新增 /api/codex-build/:runId/events 和 /api/codex-build/:runId/artifacts 两个调试接口，分别调 Codex API 的 GET /runs/{id}/events 和 GET /runs/{id}/artifacts。/api/codex-build/:runId/result 返回新增 artifactName 字段（标记 HTML 来源是 artifact 下载还是 final_response fallback）。",
    ],
  },
  {
    version: "1.3.2",
