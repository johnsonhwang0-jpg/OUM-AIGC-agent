...
- `workspace-write` requires the token to be explicitly allowed on the server. Current delivered tokens on `tangyinx_core` are allowed to use it.
- POST requests are rate-limited per token. Current default is `20` POST requests per minute.
- Server-side retention cleanup is enabled. Current default is to remove runs, project messages, projects, audit rows, workspaces, and event files older than `90` days.
- File artifacts are supported. Codex must write files under `output/` inside the run workspace for the API to expose them.
- Current artifact size limit is `50MB` per file.
...
sandbox           optional string, "read-only" or "workspace-write"; use "read-only" by default.
                  "workspace-write" requires server-side token permission.
...
List files generated by a completed run. The service only exposes files under the run workspace's `output/` directory.

```bash
curl -sS https://codex-api.tangyinx.com/runs/<run_id>/artifacts \
  -H "Authorization: Bearer <TOKEN>"
```
  ...
  "artifacts": [
    {
      ...
      "download_url": "/runs/<run_id>/artifacts/Y2hhcnQucG5n/download"
    }
  ]
}
```
...
```text
csv docx gif jpeg jpg json md pdf png pptx svg txt webp xlsx zip
```

### `GET /runs/{run_id}/artifacts/{artifact_id}/download`

Download one artifact. Only artifacts belonging to the current token's run are accessible.

```bash
curl -L -o chart.png \
  https://codex-api.tangyinx.com/runs/<run_id>/artifacts/<artifact_id>/download \
  -H "Authorization: Bearer <TOKEN>"
```

Prompting rule for AI coding agents:

```text
If you need to return files to the user, create them under ./output/ and mention the filenames in the final response.
```
...
```json
{
  "prompt": "Create a small markdown report and save it as output/report.md. Reply with a short summary and the filename.",
  "sandbox": "workspace-write",
  "timeout_seconds": 120
}
```

For browser/public frontend integrations, proxy artifact downloads through your backend. Do not expose the bearer token in download URLs.
...
403 Forbidden          token is not allowed to use workspace-write
404 Not Found          run not found, or belongs to another token
429 Too Many Requests  token exceeded the server-side POST rate limit
...
- `403`: fall back to `read-only` or ask the service owner to enable `allow_workspace_write` for this token.
- `404`: show "Run not found" and do not reveal tenant details.
- `429`: show a rate-limit message and let the user retry after a short wait. Do not auto-submit a duplicate run.
...
- Keep product-level rate limiting even though the Codex API also has per-token POST limiting.
