...
- 2026-06-29：客户 milestone 演示视频采用约 5 分钟英文独立成片：Johnson 全程不出镜，以姓名/职位动态图形卡和英文旁白表明 Product Manager 身份；视频介绍 Autopilot/Copilot 双模式和五阶段链路（导入教材→生成切片→抽取正文/图片/表格/练习→生成情景脚本→生成互动 HTML），随后诚实说明当前视觉差距，并用 `demovideo/attachment1.mp4` 展示尚未接入系统的独立视觉实验；所有视频制作素材统一放在 `demovideo/`。
- 2026-06-29：演示视频使用系统中已有的完整项目做状态巡览，不新建项目、不重新上传教材、不触发切片/抽取/脚本/HTML 的 AI 生成任务；实时生成等待时间长且增加演示风险，视频必须诚实展示已完成结果而非伪装实时执行。
- 2026-06-29：演示数据固定为：`proj-1782457894902-yyri1s5` 展示 Autopilot 全流程（数据库核验 20 个切片均有提取、脚本和 HTML）；`proj-1782108211460-12opwih` 展示 Copilot；视觉前后对比使用其 S3（模块 `mod-3-1782108846651`）当前 HTML 与 `demovideo/attachment1.mp4`，两者基于同一切片。
- 2026-07-01：**架构决策（Codex 异步轮询）**：Step 5 Build App 的 Codex 模式从同步 fetch 改为异步轮询架构。根因：Codex 是异步 Agent（30s-30min），原 `/api/generate-app-code` 同步 `await callCodexRun()` 导致 HTTP 请求长时间 pending 无数据流，被浏览器/Node.js keepAlive/requestTimeout 中止报 `net::ERR_ABORTED`。新架构三步走：①`POST /api/codex-build/start` 创建 Codex run 立即返回 runId；②前端每 3s 轮询 `GET /api/codex-build/:runId/status` 更新状态（queued/running/completed/failed）+ elapsed 秒数；③status=completed 后 `GET /api/codex-build/:runId/result` 拉取结果。服务端拆分 `callCodexRun` 为 `startCodexRun`/`pollCodexRun`/`getCodexRunResult` 三个独立函数（[server.ts#L368-L456](file:///Users/johnsonhwang/Desktop/OUM-AIGC-V2-immersive-simulation/booktogame-ai-agent/server.ts#L368)）。前端 `handleGenerateFinalApp` 按 `buildMode` 分支：codex 走异步轮询，api 保持原同步逻辑（[App.tsx#L2301](file:///Users/johnsonhwang/Desktop/OUM-AIGC-V2-immersive-simulation/booktogame-ai-agent/src/App.tsx#L2301)）。新增状态 `codexRunId`/`codexRunStatus`/`codexRunElapsed` 驱动调试面板进度显示。**教训：长耗时异步 Agent 不能用同步 HTTP fetch 等待，必须拆分为 start/poll/result 三段式。** 旧的 `/api/generate-app-code?mode=codex` 仍保留向后兼容，但前端不再调用。
- 2026-07-01：**架构决策（Step 5 布局重构）**：移除 Build App 顶部占空间的 `Gamepad2` 横幅，改为顶部紧凑栏（切片信息 + API/Codex 引擎切换 + 构建按钮 + 调试齿轮一行排列），Scene Simulation Game 占满下方剩余空间。Codex 模式下在游戏区上方显示可折叠调试面板（runId + Prompt + 原始返回 + 状态徽章）。新增 `codexDebugOpen`（默认展开）和 `lastCodexRaw`（存 Codex 原始返回）状态。
- 2026-07-01：**架构决策（Codex Agent 范式重构 v1.3.3）**：诊断到 `Codex run running after 181s` + 黑屏的根因是**范式错配**——Codex 是异步 Agent（workspace-write 模式下会把 HTML 写到文件），但原架构期望 `final_response` 直接返回 HTML 字符串，导致 `extractValidHtml` 找不到 `<!DOCTYPE html>`。修复：①新增独立 `codex-build` aiEntry（Prompt Management 第 4 个分区），system/user prompt 明确要求 Codex 把 HTML 写到 `output/index.html`，`final_response` 只返回文件名+简短总结；②`getCodexRunResult` 优先调 `/artifacts` 列文件 → 下载 HTML artifact 内容，fallback 到 `final_response`（兼容旧 LLM 范式）；③`buildCodexPrompt` 移除末尾的『直接输出 HTML 字符串』约束（与 Agent 范式冲突）；④前端按 `buildMode` 分支加载 `codex-build` 或 `app-code` prompt，两套完全解耦，API 模式作为兜底不动。**教训：Agent 调用必须用 artifact 机制获取文件产物，不能期望 final_response 返回代码字符串。**
- 2026-07-01：**架构决策（Codex Debug 面板增强）**：新增 `codexEvents`/`codexArtifacts`/`codexArtifactName` 状态。轮询 status 时并行拉 `/api/codex-build/:runId/events`（不阻塞，失败容忍），展示 Agent 最近 15 条事件流（type + message/text）。完成后拉 `/api/codex-build/:runId/artifacts` 列出产物文件（名称+大小），标记实际下载的 HTML artifact。后端新增 `/events` 和 `/artifacts` 两个调试接口，`/result` 返回新增 `artifactName` 字段标记 HTML 来源。
